...
user <- read.table("TEMP_DX_TELECOM_USER_SEQ21.txt",sep = ",",header = TRUE)
dim(user)
names(user)
...
#全7周
#sql_7 <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ21"
#dtest_7 <- dbGetQuery(con, sql_7)
#filename_7 <- "TEMP_DX_TELECOM_DATA_SEQ21.txt"
#write.table(dtest_7, file = filename_7, sep = ",", quote = TRUE, na = "", row.names = TRUE)

seq_7 <- read.table("TEMP_DX_TELECOM_DATA_SEQ21.txt",sep = ",",header = TRUE)
names(seq_7)
dim(seq_7)
...
#训练集前6周
#sql <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ22"
#dtest <- dbGetQuery(con, sql)
#filename <- "TEMP_DX_TELECOM_DATA_SEQ22.txt"
#write.table(dtest, file = filename, sep = ",", quote = TRUE, na = "", row.names = TRUE)
seq <- read.table("TEMP_DX_TELECOM_DATA_SEQ22.txt",sep = ",",header = TRUE)
names(seq)
dim(seq)
...
vd1_7 <- all_7[which(all_7$uc_7 =="100009" ),] #
vd1_7
