...
sql1 <- "SELECT * FROM TEMP_DX_TELECOM_USER_SEQ31"
dtest1 <- dbGetQuery(con, sql1)
write.table(dtest1, file = "TEMP_DX_TELECOM_USER_SEQ31.txt", sep = ",", quote = TRUE, na = "", row.names = TRUE)

user <- read.table("TEMP_DX_TELECOM_USER_SEQ31.txt",sep = ",",header = TRUE)
dim(user)
names(user)
...
#全7周
sql_7 <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ31"
dtest_7 <- dbGetQuery(con, sql_7)
filename_7 <- "TEMP_DX_TELECOM_DATA_SEQ31.txt"
write.table(dtest_7, file = filename_7, sep = ",", quote = TRUE, na = "", row.names = TRUE)

seq_7 <- read.table("TEMP_DX_TELECOM_DATA_SEQ31.txt",sep = ",",header = TRUE)
names(seq_7)
dim(seq_7)
...
#训练集前6周
sql <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ32"
dtest <- dbGetQuery(con, sql)
filename <- "TEMP_DX_TELECOM_DATA_SEQ32.txt"
write.table(dtest, file = filename, sep = ",", quote = TRUE, na = "", row.names = TRUE)
seq <- read.table("TEMP_DX_TELECOM_DATA_SEQ32.txt",sep = ",",header = TRUE)
names(seq)
dim(seq)
...
vd1_7 <- all_7[which(all_7$uc_7 =="607860" ),] #
vd1_7
...
floor(pred2$fit)
floor(pred3$fit)
sum((vd1_7$cnt_7 -round(pred1$fit)))
sum((vd1_7$cnt_7 - round(pred2$fit)))
#