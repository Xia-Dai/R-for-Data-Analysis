...
sql1 <- "SELECT * FROM TEMP_DX_TELECOM_USER_SEQ101"
dtest1 <- dbGetQuery(con, sql1)
write.table(dtest1, file = "TEMP_DX_TELECOM_USER_SEQ101.txt", sep = ",", quote = TRUE, na = "", row.names = TRUE)

user <- read.table("TEMP_DX_TELECOM_USER_SEQ101.txt",sep = ",",header = TRUE)
dim(user)
names(user)
...
n
#全7周
sql_7 <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ101"
dtest_7 <- dbGetQuery(con, sql_7)
filename_7 <- "TEMP_DX_TELECOM_DATA_SEQ101.txt"
write.table(dtest_7, file = filename_7, sep = ",", quote = TRUE, na = "", row.names = TRUE)

seq_7 <- read.table("TEMP_DX_TELECOM_DATA_SEQ101.txt",sep = ",",header = TRUE)
names(seq_7)
dim(seq_7)
...
#训练集前6周
sql <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ102"
dtest <- dbGetQuery(con, sql)
filename <- "TEMP_DX_TELECOM_DATA_SEQ102.txt"
write.table(dtest, file = filename, sep = ",", quote = TRUE, na = "", row.names = TRUE)
seq <- read.table("TEMP_DX_TELECOM_DATA_SEQ102.txt",sep = ",",header = TRUE)
names(seq)
dim(seq)
...
par(mfrow=c(1,2))
vd1_7 <- all_7[which(all_7$uc_7 =="776555" ),] #
vd1_7
...
semi.fit1 <- spm(yc ~ f(xw,knots=knots.sem1,spar=0.16)) # df 自由度 spar 光滑参数 knots 分段间隔 ,df=30,spar=1,basis="trunc.poly"
semi.fit2 <- spm(yc ~ f(xw,knots=knots.sem2,spar=0.17)) # 上周值多时选择０.１６５，值少时选择小值０.１４８
semi.fit3 <- spm(yc ~ f(xw,knots=knots.sem3,spar=0.18)) # 上周值多时选择０.１６５，值少时选择小值０.１４８
...
plot(xw,resid(semi.fit3),xlab="ts",ylab ="res2",col=2)
...
pred3<-predict(semi.fit3,newdata = next_w,se=TRUE,interval ="prediction",level=0.95)
...
sum(abs(vd1_7[which(vd1_7$ts ==seq(43,49,by=1)),4]- round(pred3$fit)))
#