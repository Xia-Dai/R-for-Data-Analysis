...
sql1 <- "SELECT * FROM TEMP_DX_TELECOM_USER_SEQ91"
dtest1 <- dbGetQuery(con, sql1)
write.table(dtest1, file = "TEMP_DX_TELECOM_USER_SEQ91.txt", sep = ",", quote = TRUE, na = "", row.names = TRUE)

user <- read.table("TEMP_DX_TELECOM_USER_SEQ91.txt",sep = ",",header = TRUE)
dim(user)
names(user)
...
#全7周
sql_7 <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ91"
dtest_7 <- dbGetQuery(con, sql_7)
filename_7 <- "TEMP_DX_TELECOM_DATA_SEQ91.txt"
write.table(dtest_7, file = filename_7, sep = ",", quote = TRUE, na = "", row.names = TRUE)

seq_7 <- read.table("TEMP_DX_TELECOM_DATA_SEQ91.txt",sep = ",",header = TRUE)
names(seq_7)
dim(seq_7)
...
#训练集前6周
sql <- "SELECT * FROM TEMP_DX_TELECOM_DATA_SEQ92"
dtest <- dbGetQuery(con, sql)
filename <- "TEMP_DX_TELECOM_DATA_SEQ92.txt"
write.table(dtest, file = filename, sep = ",", quote = TRUE, na = "", row.names = TRUE)
seq <- read.table("TEMP_DX_TELECOM_DATA_SEQ92.txt",sep = ",",header = TRUE)
names(seq)
dim(seq)
...
par(mfrow=c(1,2))
vd1_7 <- all_7[which(all_7$uc_7 =="280599" ),] #
vd1_7
...
semi.fit2 <- spm(yc ~ f(xw,knots=knots.sem2,spar=0.16)) # 上周值多时选择０.１６５，值少时选择小值０.１４８
